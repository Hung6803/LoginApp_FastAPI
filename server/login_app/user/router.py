...
from login_app.user import services, validator, token
from login_app.user.models import User
from login_app.user.oauth2 import get_current_user
...
async def create_user_registration(request: schema.User, database: Session = Depends(database.get_db)):
...
async def get_all_user(database: Session = Depends(database.get_db)):
